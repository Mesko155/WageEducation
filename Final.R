...
a <- data.frame(fit = final1_ns$fitted.values, res=final1_ns$residuals)
...
finalNoExp <-   
  main %>%
  ivreg(log(wage) ~ iq + I(factor(state)) | edu | edu_par + dist, data = .)
...
b <-   
  main %>%
  ivreg(edu ~ iq + edu_par + dist, data = .)
...
# LH F test 575 === Weak instrument stat 575
